...
int set_value(int key, char *value1, float value2){
	...
	if(reply.fcode==-1) return -1;
	if(reply.fcode==1){
		printf("[ERROR] Cannot insert value with key %d: Repeated key\n",key);
		return -1;
	}
...
int get_value(int key, char **value1, float *value2){
	...
	if(reply.fcode==1){
		printf("[ERROR] Cannot get element with key %d: Not existing key.\n",key);
		return -1;
	}
...
int modify_value(int key, char *value1, float *value2){
	...
	if(reply.fcode==-1) return -1;
	if(reply.fcode==1){
		printf("[ERROR] Cannot modify element with key %d: Not existing key.\n",key);
		return -1;
	}
...
int delete_key(int key){
	...
	if(reply.fcode==-1) return -1;
	if(reply.fcode==1){
		printf("[ERROR] Cannot delete element with key %d: Not existing key.\n",key);
		return -1;
	}
...
int num_items(){
	...
	if(close_conection()==-1) return -1;
	printf("Number of items stored calculated.\n");
	return result;
}
